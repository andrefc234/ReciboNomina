...
function EmpleadosTable() {
  ...
  useEffect(() => {
    fetch(`http://${process.env.IP}:5000/api/v1/empleado`)
  ...
  const handleDeleteEmpleado = (id: string) => {
    fetch(`http://${process.env.IP}:5000/api/v1/empleado/${id}`, {
      method: 'DELETE',
    })
      .then((response) => response.json())
  ...
  const handleUpdateEmpleado = () => {
    fetch(`http://${process.env.IP}:5000/api/v1/empleado/${updateEmpleado._id}`, {
      method: 'PUT',
      headers: {
